...
# КОНФИГУРАЦИЯ
# =============================================================================
TOKEN = "8780781533:AAE79vv1FsRpYeeZi882C8rE2KpuXuw-Qzg, 8548607252:AAFFzd__XttKj6GxcFh_IygRQbgTu7-xL68"
...
class CryptoBotClient:
    ...
    async def delete_invoice(self, invoice_id: int) -> bool:
        ...
        try:
            ...
            log_message(f"Ошибка удаления инвойса: {e}")
            return False
...
class CB:
    ...
    SUPPORT_DONATE_REFRESH = "s:donate:refresh"
    SUPPORT_DONATE_DELETE = "s:donate:delete"
...
def kb_support_donate_active(invoice_url: Optional[str] = None) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    if invoice_url:
        rows.append([InlineKeyboardButton(text="💳 Открыть счёт в CryptoBot", url=invoice_url)])
        rows.append(
            [
                InlineKeyboardButton(text="🔄 Проверить оплату", callback_data=CB.SUPPORT_DONATE_REFRESH),
                InlineKeyboardButton(text="🗑 Удалить счёт", callback_data=CB.SUPPORT_DONATE_DELETE),
            ]
        )
    else:
        rows.append([InlineKeyboardButton(text="🗑 Удалить счёт", callback_data=CB.SUPPORT_DONATE_DELETE)])
    rows.append([InlineKeyboardButton(text="Назад", callback_data=CB.SUPPORT_DONATE_CANCEL)])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def check_support_donation_payment(
    ...
    while True:
        await asyncio.sleep(PAYMENT_CHECK_INTERVAL)
        active = awaiting_support_donation_payment.get(user_id) or {}
        try:
            active_invoice_id = int(active.get("invoice_id") or 0)
        except Exception:
            active_invoice_id = 0
...
async def cb_profile_redeem(call: CallbackQuery, bot: Bot):
    ...
    await safe_edit_text(
        ...
        (
            ...
            "Введите количество очков, которое хотите обменять."
        ),
...
async def cb_menu_support_donate(call: CallbackQuery, bot: Bot):
    ...
    if active:
        url = active.get("invoice_url") or ""
        if str(active.get("type") or "").strip().lower() == "stars":
            stars_amount = active.get("amount")
            stars_suffix = f" ({int(stars_amount)} ⭐)" if isinstance(stars_amount, int) and stars_amount > 0 else ""
            text = (
                "🤝 <b>Поддержка бота</b>\n\n"
                f"У вас уже есть активный счёт Stars{stars_suffix}.\n"
                "Оплатите его в сообщении со счётом или удалите, чтобы создать новый."
            )
        elif url:
            display_amount = str(active.get("display_amount") or "").strip()
            try:
                amount_usd = float(active.get("amount_usd") or 0.0)
            except Exception:
                amount_usd = 0.0
            amt_line = ""
            if display_amount and amount_usd > 0:
                amt_line = f"\nСумма: {display_amount} (≈ ${amount_usd:.2f})"
            text = (
                "🤝 <b>Поддержка бота</b>\n\n"
                "У вас уже есть активный счёт в CryptoBot.\n"
                f"Оплатите его или удалите, чтобы создать новый.{amt_line}"
            )
        else:
            text = (
                "🤝 <b>Поддержка бота</b>\n\n"
        ...
        await safe_edit_text(
            ...
            call.message.message_id,
            text,
            kb_support_donate_active(url if url else None),
        )
        return
    awaiting_support_donation.pop(call.from_user.id, None)
    await safe_edit_text(
...
@router.callback_query(F.data == CB.SUPPORT_DONATE_DELETE)
async def cb_support_donate_delete(call: CallbackQuery, bot: Bot):
    await safe_answer(call)
    uid = call.from_user.id
    active = awaiting_support_donation_payment.get(uid)
    awaiting_support_donation.pop(uid, None)
    awaiting_support_donation_payment.pop(uid, None)
    if active:
        try:
            inv_id = int(active.get("invoice_id") or 0)
        except Exception:
            inv_id = 0
        if inv_id > 0 and cryptobot.enabled:
            await cryptobot.delete_invoice(inv_id)
    await safe_edit_text(
        bot,
        call.message.chat.id,
        call.message.message_id,
...
@router.callback_query(F.data == CB.SUPPORT_DONATE_REFRESH)
async def cb_support_donate_refresh(call: CallbackQuery, bot: Bot):
    await safe_answer(call)
    uid = call.from_user.id
    active = awaiting_support_donation_payment.get(uid) or {}
    invoice_id = int(active.get("invoice_id") or 0)
    if invoice_id <= 0 or not cryptobot.enabled:
        await safe_answer(call, "Нет активного счёта CryptoBot.", show_alert=True)
        return
    try:
        paid, _ = await cryptobot.check_payment(invoice_id)
    except Exception:
        paid = False
    if not paid:
        await safe_answer(call, "Платёж пока не найден. Попробуйте ещё раз через пару секунд.", show_alert=True)
        return
    amount_usd = float(active.get("amount_usd") or 0.0)
    display_amount = str(active.get("display_amount") or "").strip()
    awaiting_support_donation_payment.pop(uid, None)
    await safe_edit_text(
        ...
        call.message.chat.id,
        call.message.message_id,
        f"✅ Спасибо за поддержку! Платёж {display_amount or 'получен'} подтверждён.",
        kb_menu(),
    )
    for admin_id in ADMIN_USER_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"🤝 Поддержка бота: {display_amount or 'оплата'} (≈ ${amount_usd:.2f}) от пользователя {uid}"
                if amount_usd > 0 else f"🤝 Поддержка бота: {display_amount or 'оплата'} от пользователя {uid}",
            )
        except Exception:
            pass
...
async def msg_any(message: Message, bot: Bot):
    ...
    if uid in awaiting_points_redeem:
        ...
        raw = text.replace(" ", "")
        if not raw.isdigit():
            await message.answer("Введите количество очков целым числом (например 1000).")
            return
        points_needed = int(raw)
        balance = db.get_points_profile(uid).get("balance", 0)
        if points_needed <= 0:
            await message.answer("Введите количество очков больше 0.")
            return
        if balance < points_needed:
            ...
            return
        amount_usd = (Decimal(points_needed) / Decimal(REDEEM_POINTS_PER_USD)).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        if amount_usd <= 0:
            await message.answer("Сумма по курсу слишком маленькая. Укажите больше очков.")
            return
        awaiting_points_redeem.pop(uid, None)
        check = None
        last_err = None
    ...
    if uid in awaiting_support_donation:
        ...
        if cur == "stars":
            ...
            if uid in awaiting_support_donation_payment:
                ...
                if data.get("invoice_url"):
                    await message.answer(
                        "У вас уже есть активный счёт. Дождитесь оплаты или удалите его, чтобы создать новый.",
                        reply_markup=kb_support_donate_active(str(data.get("invoice_url") or "").strip() or None),
                    )
                    return
                if now_ts() - created_at < 600:
                    await message.answer(
                        "У вас уже есть активный счёт Stars. Дождитесь оплаты или удалите его, чтобы создать новый.",
                        reply_markup=kb_support_donate_active(None),
                    )
                    return
                awaiting_support_donation_payment.pop(uid, None)
            awaiting_support_donation.pop(uid, None)
            ...
            await message.answer(
                "Счёт Stars создан. Оплатите его в появившемся сообщении.\n\n"
                "Если хотите создать новый — удалите текущий счёт.",
                reply_markup=kb_support_donate_active(None),
            )
            return
        if cur in ("usd", "rub") and not cryptobot.enabled:
        ...
        if uid in awaiting_support_donation_payment:
            ...
            if url:
                await message.answer(
                    "У вас уже есть активный счёт в CryptoBot. Оплатите его или удалите, чтобы создать новый.",
                    reply_markup=kb_support_donate_active(str(url).strip() or None),
                )
            else:
                await message.answer(
                    "У вас уже есть активный счёт. Дождитесь оплаты или удалите его, чтобы создать новый.",
        ...
        await message.answer(
            ...
            "Откройте счёт в CryptoBot и оплатите.\n\n"
            "Если передумали — удалите счёт и создайте новый."
            ,
            reply_markup=kb_support_donate_active(invoice_url),
        )
        asyncio.create_task(
